	...
	jQuery.fn.extend({
		...
		css: function(key, value) {
			return jQuery.access(this, function(key, value) {
				...
				return value !== undefined ? jQuery.style(this, key, value) : window.getComputedStyle(this).getPropertyValue(
					key);
			}, key, value);
		},
		addClass: function(value) { //this
			var len = this.length;
			var classes, elem, cur, j, clazz, i = 0;
			var proceed = typeof value === "string" && value;
			if (proceed) {
				classes = value.match(/\S+/g) || []; //"box item"
				for (; i < len; i++) {
					elem = this[i];
