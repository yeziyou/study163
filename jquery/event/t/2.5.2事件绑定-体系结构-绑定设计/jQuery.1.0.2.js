		...
		each: function(object, callback, args) {
			...
			if (args) {
				...
				if (length === undefined) {
					...
					for (; i < length;) {
						// object[i] 为element对象元素（节点本身） => 原生element
						// 这里i 和 object[i++]传入似乎没啥用 ？？？
	...
	Data.prototype = {
		key: function(elem) {
			...
			if (!unlock) {
				...
				//DOM   =>  elem.jQuery101089554822917892030.7449198463843298 = 1;
				Object.defineProperties(elem, descriptor);
				console.log(this.expando, elem[this.expando]);
			}
			//确保缓存对象记录信息
	...
	jQuery.fn.extend({
		...
		on: function(types, fn) {
			...
			return this.each(function() {
				// this  element对象
				// types click
				// fn function(){console.log("loading...")}
				console.log(this);
